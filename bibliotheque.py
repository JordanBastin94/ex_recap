...
def filter_by_category(category : str, list_to_search):
    """
    ...
    search_result = []
    for book in list_to_search:
        if category in book[1][1]: 
            search_result.append(book)
    return search_result


def page_chooser(books, page=1):
    """
...
def search(params : dict):
    ...
    if "category" in params:
        list_to_search = filter_by_category(params.get("category"), list_to_search)
    if "page" in params:
        list_to_search = page_chooser(list_to_search,params.get("page"))
    else:
        list_to_search = page_chooser(list_to_search)
...
def start():
    ...
    while(keep_using):
        params = {}
        user_title = input("Search a title ? (keep empty if not) : ")
        if user_title != '':
            params.setdefault("title",user_title)
        user_category = input("Category name ? : ")
        if user_category !="":
            params.setdefault("category",user_category)
        user_page = input("Which page to show ? ")
        if user_page != "":
